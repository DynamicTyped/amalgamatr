...
class HexColor extends Color {
    constructor(protected Value: string = '') {
        ...
        if (Value !== null && Value.length > 0) {
            if (!this.validate()) throw "Not a valid hex color!";
            this.parse();
        }
    }
    ...
    public toString() {
        var rHex = this.toHex(this.Red);
        var gHex = this.toHex(this.Green);
        var bHex = this.toHex(this.Blue);
        var aHex = this.toHex(this.Alpha);
    ...
    protected parse() {
        ...
        this.Alpha = this.Value.length === 9 ? parseInt(this.Value.substr(7,2)) : 1;
    }

    protected toHex(value: number) {
        var clampedValue = Math.min(255, Math.max(0, Math.round(value)));
